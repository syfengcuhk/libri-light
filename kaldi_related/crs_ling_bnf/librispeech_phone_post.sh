...
if [ $stage -le 0 ] && [ $stop_stage -gt 0 ];then
    ...
    for set in  dev_clean dev_other  test_other test_clean; do
...
if [ $stage -le 3 ] && [ $stop_stage -gt 3 ];then
   for set in  test dev ; do #test; do
      for cond in  clean ; do # other; do
	 # (
 	  echo "starting ${set}-${cond}"
          data_file=${set}-${cond}
          input_data=$model/decode_${set}_${cond}_${lm_used}/phone_post_lm${lmscore} #bnf_for_libri_light/$data_file
	  ...
	  copy-feats ark:$input_data/phone_post_feat.ark scp,t:$foo_path/foo_${set}-${cond}.scp || exit 1;
...
if   [ $stage -le 5 ] && [ $stop_stage -gt 5 ]; then
   ...
   input_data=$model/decode_${eval_set_underscore}_${lm_used}/phone_post_lm${lmscore} #bnf_for_libri_light/$data_file
...
if  [ $stage -le 9 ] && [ $stop_stage -gt 9 ] ; then
  echo "stage 9: DNN output -> post-to-phone-post"
  data_root_path=/tudelft.net/staff-bulk/ewi/insy/SpeechLab/siyuanfeng/libri-light/kaldi_related/data_hires_conf_librispeech
  bnf_name=output.log-softmax
  eval_set=$abx_eval_set_appoint
  eval_set_hyphen=${eval_set/_/-}
  input_data=$data_root_path/$eval_set_hyphen
  output_data=$model/dnn_output_by_pdf/$eval_set
  input_ivec_dir=$model/../ivectors_libri_light_hires/$eval_set_hyphen
  tacc_folder=$model/align_${eval_set/-/_}_nj1/
  if [ ! -f $tacc_folder/final.tacc  ];then
    ali-to-post "ark:gunzip -c $tacc_folder/ali.1.gz|" ark:- | post-to-tacc $tacc_folder/final.mdl ark:- $tacc_folder/final.tacc
  fi
  steps/nnet3/make_bottleneck_features.sh \
           --cmd run.pl  --nj $nj --ivector-dir $input_ivec_dir \
           $bnf_name $input_data $output_data $model || exit 1;
 
  logprob-to-post scp:$output_data/feats.scp ark:- |  post-to-phone-post --transition-id-counts=$tacc_folder/final.tacc $model/final.mdl ark:- ark:- | post-to-feats --post-dim=42 ark:- ark,t:$output_data/$eval_set/phone_post.ark 

fi

if  [ $stage -le 10 ] && [ $stop_stage -gt 10 ] ; then
   data_file=$abx_eval_set_appoint
   eval_set_hyphen=${data_file/_/-}
#   input_data=$data_root_path/$eval_set_hyphen

   input_data=$model/dnn_output_by_pdf/$data_file #/align_${abx_eval_set_appoint/-/_}_nj1/phone_post
   output_path=$input_data/data_by_utt
   foo_path=$output_path/../
   mkdir -p $output_path
   cp data/$data_file/foo_template.scp $foo_path/foo_${abx_eval_set_appoint}.scp
   sed -i "s| PathToRep| $output_path\/|g"  $foo_path/foo_${abx_eval_set_appoint}.scp || exit 1
   copy-feats ark:$input_data/phone_post.ark scp,t:$foo_path/foo_${abx_eval_set_appoint}.scp || exit 1;
   echo "${abx_eval_set_appoint}: starting removing [ and ], will construct .npy.txt files and remove .n ones"
   if [ ! -z $output_path ] ; then
         for file in $output_path/*.n ; do
             sed -e "/\[/d" -e "s/\]//g" -e "s/^ //g" -e "s/ $//g"  $file  > ${file}py.txt;
...
if  [ $stage -le 11 ] && [ $stop_stage -gt 11 ] ; then
  echo "enter (libri-light)"
   ...
   eval_set=$abx_eval_set_appoint
   eval_set_underscore=${eval_set/-/_}
   input_data=$model/dnn_output_by_pdf/${eval_set}  
   source_dir=$input_data/data_by_utt_npy
#   source_dir=$model/bnf_for_libri_light_by_utt/${eval_set}_npy/
   target_dir=$source_dir/../eval${distance_mode_abx}/
   mkdir -p $target_dir || exit 1;
