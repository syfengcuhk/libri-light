...
if [ $stage -le 0 ] && [ $stop_stage -gt 0 ];then
    for set in  dev-other dev-clean test-other test-clean; do
        utils/copy_data_dir.sh data/$set data_fbank/$set || exit 1;
        steps/make_fbank.sh --nj $nj data_fbank/$set || exit 1;
        steps/compute_cmvn_stats.sh data_fbank/$set || exit 1;
    done
fi
   ...
   for set in dev test; do
	 ...
	 # (
 	  echo "starting ${set}-${cond}"
          data_file=${set}-${cond}
	  output_path=/tudelft.net/staff-bulk/ewi/insy/SpeechLab/siyuanfeng/libri-light/kaldi_related/data_fbank_by_utt${cmvn_suffix}/$data_file
	  ...
	  #apply-cmvn --utt2spk=ark:data_fbank/$data_file/utt2spk scp:data_fbank/$data_file/cmvn.scp scp:data_fbank/$data_file/feats.scp scp,t:$foo_path/foo_${set}-${cond}.scp || exit 1;
	  copy-feats scp:data_fbank/$data_file/feats.scp scp,t:$foo_path/foo_${set}-${cond}.scp || exit 1;
