...
if [ $stage -le 0 ] && [ $stop_stage -gt 0 ]; then
  ...
  ivector_extractor=${aidatatang_root_dir}/exp/nnet3/extractor
#/tudelft.net/staff-bulk/ewi/insy/SpeechLab/siyuanfeng/software/kaldi/egs/cgn/s5/exp/nnet3/extractor
  ...
  for lang in english french mandarin; do
    ...
    steps/online/nnet2/extract_ivectors_online.sh --nj $nj_ivector \
...
if [ $stage -le 1 ] && [ $stop_stage -gt 1 ]; then
  ...
  for lang in  english french mandarin  ; do
    ...
    input_ivec_dir=$aidatatang_root_dir/exp/nnet3/ivectors_zr17_hires/test/$lang/$dur
