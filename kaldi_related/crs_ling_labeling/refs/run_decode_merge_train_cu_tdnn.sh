...
if [ $stage -le 3 ]; then
	...
	if [ ! -f $model_dir/decode_zerospeech_train_${lang}/lat.1.gz ]; then
		...
		steps/nnet3/decode.sh --nj 8 --cmd "$decode_cmd" --skip-scoring true --num-threads 4  \
	...
	gzip -c $model_dir/decode_zerospeech_train_${lang}/best_path.ali > $model_dir/decode_zerospeech_train_${lang}/ali.1.gz || exit 1;
	mv $model_dir/decode_zerospeech_train_${lang}/num_jobs $model_dir/decode_zerospeech_train_${lang}/num_jobs_decode
	echo "1" > $model_dir/decode_zerospeech_train_${lang}/num_jobs || exit 1;
	# done
