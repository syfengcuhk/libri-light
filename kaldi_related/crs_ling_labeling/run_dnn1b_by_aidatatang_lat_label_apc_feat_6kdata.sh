#!/bin/bash
# Originated from run_dnn1b_by_cgn_lat_label.sh 
...
apc_notation_train_set=unlab_6k_full
...
min_seg_len=1.55
xent_regularize=0.1
train_set=train_unlab_6k
train_subset=
lat_generator_acwt=10.0
gmm=tri5a  # the gmm for the target data
num_threads_ubm=32
...
lat_generator_model=$aidatatang_root_dir/exp/chain/bnf_layer/tdnn_1a_input_no_ivec_sp
input_feat_affix=
...
which_apc_layer=-1
apc_dump_train_name=train-unlab_6k${train_subset}_${which_apc_layer}
gmm_dir=$aidatatang_root_dir/exp/$gmm
...
dir=exp/chain${nnet3_affix}_aidatatang_lat_label/${train_set}${train_subset}/lat_gen_acwt${lat_generator_acwt}/apc_feats_${apc_notation_train_set}/${apc_notation_config}/dnn${tdnn_affix}_bi_epoch${num_epochs}${input_feat_affix}
data_root_dir=/tudelft.net/staff-bulk/ewi/insy/SpeechLab/siyuanfeng/Autoregressive-Predictive-Coding/exp/${apc_notation_train_set}/${apc_notation_config}.dir/extracted_feats_no_pad
train_data_dir=${data_root_dir}/$apc_dump_train_name/data_for_kaldi
...
common_egs_dir= #exp/chain_aidatatang_lat_label/${train_set}${train_subset}/lat_gen_acwt${lat_generator_acwt}/apc_feats_${apc_notation_train_set}/${apc_notation_config}/dnn1b_bi_epoch3/egs
...
if [ $stage -le 15 ] && [  $stop_stage -gt 15 ]  ; then
  ...
  steps/align_fmllr_lats.sh --stage $align_fmllr_lats_stage --nj $nj --cmd "$train_cmd" ${lores_train_data_dir} \
    $aidatatang_root_dir/data/lang $gmm_dir $lat_dir
...
if [ $stage -le 16 ] && [ $stop_stage -gt 16 ] ; then
  ...
  steps/nnet3/chain/build_tree.sh --frame-subsampling-factor 3 \
      ...
      --cmd "$train_cmd" 4000 ${lores_train_data_dir} $aidatatang_root_dir/data/lang_chain $ali_dir $tree_dir
fi
...
if [ $stage -le 18 ] && [ $stop_stage -gt 18 ] ; then
 ...
 steps/nnet3/chain/train.py --stage $train_stage --exit-stage $train_exit_stage \
    ...
    --feat.online-ivector-dir $aidatatang_root_dir/$rel_train_ivector_dir \
    ...
    --egs.chunk-width 150 --egs.stage $egs_stage   \
...
if [ $stage -le 21 ] && [ $stop_stage -gt 21 ];then
  ...
  for set in  dev-other dev-clean test-other test-clean; do
    ...
    input_ivec_dir=$aidatatang_root_dir/exp/nnet3/ivectors_libri_light_hires/$set
