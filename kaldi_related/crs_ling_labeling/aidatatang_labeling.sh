...
if [ $stage -le -1 ]  && [ $stop_stage -gt -1 ];then
    ...
    data_root_path=/tudelft.net/staff-bulk/ewi/insy/SpeechLab/siyuanfeng/libri-light/kaldi_related/data_hires_conf_aidatatang
...
if [ $stage -le 0 ] && [ $stop_stage -gt 0 ];then
    ...
    data_root_path=/tudelft.net/staff-bulk/ewi/insy/SpeechLab/siyuanfeng/libri-light/kaldi_related/data_hires_pitch_conf_aidatatang
    input_data=$data_root_path/$train_name
...
if [ $stage -le 6 ] && [ $stop_stage -gt 6 ];then
  ...
  utils/copy_data_dir.sh data/${train_name} data_plus_aidatatang_transcripts/acwt${acwt}/${train_name} || exit 1;
  cp $text_dir/text data_plus_aidatatang_transcripts/acwt${acwt}/${train_name}
  # we have to extract mfcc-pitch features from scratch to match aidatatang-gmm model
  utils/fix_data_dir.sh data_plus_aidatatang_transcripts/acwt${acwt}/${train_name}
  steps/make_mfcc_pitch.sh --nj $nj --cmd run.pl data_plus_aidatatang_transcripts/acwt${acwt}/${train_name} || exit 1;
  steps/compute_cmvn_stats.sh data_plus_aidatatang_transcripts/acwt${acwt}/${train_name} || exit 1;
...
if [ $stage -le 7 ] && [ $stop_stage -gt 7 ];then
  ...
  data_dir=data_plus_aidatatang_transcripts/acwt${acwt}/${train_name}
  ali_dir=exp/chain_aidatatang_lat_label/$train_name/lat_gen_acwt${acwt}/${train_name}_ali
  steps/align_fmllr.sh --nj $nj --cmd run.pl --stage $align_fmllr_stage  \
    $data_dir $gmm_model/../../data/lang $gmm_model $ali_dir
fi
...
if [ $stage -le 10 ] && [ $stop_stage -gt 10 ];then
  ...
  for set in dev-clean dev-other test-clean test-other; do
    ...
    utils/copy_data_dir.sh data/${set} data_plus_aidatatang_transcripts/acwt${acwt}/${set} || exit 1;
    cp $text_dir/text data_plus_aidatatang_transcripts/acwt${acwt}/${set}
    utils/fix_data_dir.sh data_plus_aidatatang_transcripts/acwt${acwt}/${set}
    steps/make_mfcc_pitch.sh --nj $nj --cmd run.pl data_plus_aidatatang_transcripts/acwt${acwt}/${set} || exit 1;
    steps/compute_cmvn_stats.sh data_plus_aidatatang_transcripts/acwt${acwt}/${set} || exit 1;
  done
  source /scratch/siyuanfeng/software/anaconda3/bin/deactivate
fi
...
#  for set in dev-clean dev-other test-clean test-other; do
    ...
    data_dir=data_plus_aidatatang_transcripts/acwt${acwt}/${set}
    ali_dir=exp/chain_aidatatang_lat_label/$set/lat_gen_acwt${acwt}/${set}_ali_nj1
    steps/align_fmllr.sh --nj 1 --cmd run.pl --stage $align_fmllr_stage  \
      $data_dir $gmm_model/../../data/lang $gmm_model $ali_dir
#  done
fi
...
#    for set in dev-clean dev-other test-clean test-other; do
      set=$eval_set_appoint
      dir=exp/chain_aidatatang_lat_label/$set/lat_gen_acwt${acwt}/${set}_ali_nj1
