...
input_feat_affix=mfcc_hires_pitch_only
dir=exp/chain${nnet3_affix}_aidatatang_lat_label/${train_set}${train_subset}/lat_gen_acwt${lat_generator_acwt}/dnn${tdnn_affix}_bi_epoch${num_epochs}_input${input_feat_affix}
train_data_dir=data_hires_pitch_conf_aidatatang/${train_set}${train_subset}

lores_train_data_dir=data_plus_aidatatang_transcripts/acwt${lat_generator_acwt}/${train_set}${train_subset}
common_egs_dir=exp/chain_aidatatang_lat_label/${train_set}${train_subset}/lat_gen_acwt${lat_generator_acwt}/dnn1b_bi_epoch2_inputmfcc_hires_pitch_only/egs
...
if [ $stage -le 18 ] && [ $stop_stage -gt 18 ] ; then
 ...
 steps/nnet3/chain/train.py --stage $train_stage --exit-stage $train_exit_stage \
    ...
    --egs.dir "$common_egs_dir" --egs.stage $egs_stage  \
    --egs.opts "--frames-overlap-per-eg 0" \
    --egs.chunk-width 150  --egs.nj $egs_nj \
...
#          output_path=$dir/bnf_for_libri_light_by_utt/$data_file
          ...
          copy-feats scp:$input_data/feats.scp scp,t:$foo_path/foo_${abx_eval_set_appoint}.scp || exit 1;
